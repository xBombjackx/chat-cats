...
(()=>{
  ...
  function connect(){
    ...
    ws.onmessage=e=>{ let m; try{ m=JSON.parse(e.data); }catch{ return; }
      ...
      else if(m.type==='config'){ if(m.maxCats) MAX_CATS=m.maxCats; if(m.env&&!envLocked) setEnv(m.env); if(m.species) SPECIES_MODE=m.species; if(m.daynight!=null&&q.get('daynight')==null){ DAYNIGHT=!!m.daynight; applyTimeOfDay(); } }
